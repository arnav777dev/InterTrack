...
const LoginComponent: React.FC = () => {
  ...
  const login = async () => {
    ...
    const response: LoginResponse = await makeUnauthenticatedPOSTRequest(
      "/auth/login",
      data
    );
    ...
    <div className="w-full h-full flex flex-col items-center bg-app-black text-white overflow-auto">
      ...
      <div className="inputRegion w-1/3 py-10 flex items-center justify-center flex-col">
        ...
        <div className=" w-full flex items-center justify-end my-8">
          <button
            className="h-2/3 flex items-center justify-center rounded-full font-semibold bg-green-500 hover:bg-green-600 p-3 px-10 mt-6"
            onClick={(e) => {
              e.preventDefault();
              login();
            }}
